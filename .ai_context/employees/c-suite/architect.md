...
- No synchronous HTTP clients → Use `httpx.AsyncClient`
...
- **Security:** `frontend/.npmrc` must include:
  ...
  engine-strict=true
  ```
  This blocks packages published <24 hours ago (supply chain protection).

### Backend (pip)
- **Security:** Run `pip-audit --strict` before releases
- **Freezing:** Use `requirements.txt` with pinned versions
