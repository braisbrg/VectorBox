...
export default function OnboardingTagsPage() {
    ...
    useEffect(() => {
        setMounted(true);
        const saved = localStorage.getItem("vb_guest_tags");
        if (saved) {
            try {
    ...
    const handleChange = (next: Record<string, TagState>) => {
        setStates(next);
        localStorage.setItem("vb_guest_tags", JSON.stringify(tagStateToPreferences(next)));
    };
        ...
        <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4 relative overflow-hidden">
            ...
            >
                <div className="text-center space-y-2">
                    <h1 className="text-3xl md:text-4xl font-black tracking-tighter font-mono uppercase">
                ...
                <div className="text-center">
                    <button onClick={() => router.push("/login")} className="text-[10px] font-mono text-zinc-700 hover:text-zinc-500 transition-colors uppercase tracking-wider">
                        ← BACK TO LOGIN
                    </button>
                </div>
            </motion.div>
