...
if [[ "$is_prod" =~ ^[Yy]$ ]]; then
    ...
    echo -e "\033[1;33m⚠️  Remember to set real values in .env:\033[0m"
    echo "   SECRET_KEY — run: openssl rand -hex 32"
    echo "   POSTGRES_PASSWORD — use a strong random password"
    echo "   All API keys (GROQ, TMDB, OMDB)"
    echo ""
else
...
if [[ "$1" == "--clean" ]]; then
    ...
    $COMPOSE_CMD down --remove-orphans || echo "Cleanup had minor issues, proceeding anyway..."
fi
...
$COMPOSE_CMD exec -T backend python scripts/wait_for_db.py
...
$COMPOSE_CMD exec -T backend python scripts/seed_db.py --limit 200
...
$COMPOSE_CMD exec -T backend python scripts/create_qdrant_indexes.py
