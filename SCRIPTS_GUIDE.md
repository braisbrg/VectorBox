...
| **`check_embeddings.py`** | **Embedding Sanity Check.** Compares stored Qdrant vectors against a reference MiniLM embedding built from `title + year + genres + directors`. Flags movies below cosine threshold as likely-corrupt. Flags: `--update-db` persist score, `--fix` re-enrich flagged, `--user-id` scope to one user, `--tmdb-id` check a single movie, `--recheck` re-run on movies that already have a score (default skips them), `--verbose` print reference text + first 5 dims of stored/reference vectors. | `docker-compose exec backend python scripts/check_embeddings.py --tmdb-id 129 --verbose --recheck` |
