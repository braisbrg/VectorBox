...
| **`check_embeddings.py`** | **Embedding Sanity Check.** Compares stored Qdrant vectors against a reference MiniLM embedding built from `title + year + genres + directors`. Flags movies below cosine threshold as likely-corrupt. Use `--update-db` to persist `embedding_quality_score`, `--fix` to re-enrich flagged via Scout, `--user-id` to scope to one user's library. | `docker-compose exec backend python scripts/check_embeddings.py --limit 200 --update-db` |
